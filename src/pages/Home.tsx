...
import { ArrowRight, MapPin, Hammer, ChevronLeft, ChevronRight } from 'lucide-react';
    ...
    <div>
        ...
        <div className="relative max-w-7xl mx-auto px-4 h-full flex items-center">
          <div className="text-white">
            <h1 className="text-5xl font-bold mb-4">
              <span className="text-zwolinski-burgundy">Building Dreams,</span><br />
              Delivering Excellence
            </h1>
            <p className="text-xl mb-8 max-w-2xl">
              Zwolinski Quality Construction brings over 20 years of experience in residential and commercial construction to Perkasie and surrounding areas.
            </p>
            <a 
              href="/contact"
      ...
      <section className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-12 text-zwolinski-navy">Our Services</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {['Residential Construction', 'Commercial Construction', 'Renovations'].map((service) => (
              <div key={service} className="bg-white p-6 rounded-lg shadow-md border-t-4 border-zwolinski-burgundy">
                <h3 className="text-xl font-semibold mb-4 text-zwolinski-navy">{service}</h3>
                <p className="text-gray-600 mb-4">
                  Professional {service.toLowerCase()} services tailored to your needs and specifications.
                </p>
                <a href="/services" className="text-zwolinski-burgundy hover:text-opacity-80 flex items-center">
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="relative">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 overflow-hidden">
              <div 
                className="col-span-full grid grid-cols-1 md:grid-cols-3 gap-8 transition-all duration-700 ease-in-out transform"
                style={{
              ...
              {currentTestimonials.map((testimonial, index) => (
                <div 
                  key={index} 
                  className="bg-white p-6 rounded-lg shadow-md border border-gray-100 transition-all duration-700 transform hover:scale-105 hover:shadow-lg hover:border-zwolinski-burgundy"
                >
                  <div className="flex items-center gap-4 mb-4">
                    <div className="flex items-center text-zwolinski-burgundy">
                      <MapPin className="h-5 w-5" />
                      <span className="ml-1 text-sm">{testimonial.location}</span>
                    </div>
                    <div className="flex items-center text-zwolinski-burgundy">
                      <Hammer className="h-5 w-5" />
                      <span className="ml-1 text-sm">{testimonial.projectType}</span>
                    </div>
                  </div>
                  <p className="text-gray-600 mb-4">"{testimonial.text}"</p>
                  {testimonial.name && (
                    <p className="font-semibold text-zwolinski-navy">{testimonial.name}</p>
                  )}
                </div>
              ))}
              </div>
              ...
              {[...Array(totalPages)].map((_, i) => (
                <button
                  ...
                  className={`h-2 w-2 rounded-full transition-all duration-300 ${
                    i === currentPage ? 'bg-zwolinski-burgundy w-4' : 'bg-gray-300'
                  }`}
                />
              ))}
            </div>
